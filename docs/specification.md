...
目標設定会議（10月〜、部長陣が各メンバーの目標内容を確認する会議）で修正が必要と判断されたメンバーに対して、部長以上のロール（MANAGER）が承認済み目標を差し戻す機能。

**フロー:**

```
社員が目標設定 → 上長が承認（APPROVED）
  ↓
目標設定会議（部長陣が内容確認）
  ↓ 修正が必要な場合
部長が「会議差し戻し」を実行 → MEETING_REJECTED
  （差し戻しコメント入力必須）
  ↓ 社員に通知
社員が目標を修正 → 再申請（PENDING_APPROVAL）
  ↓ 上長が再承認
APPROVED（確定）
```
...
- 操作可能ステータス: `APPROVED` のみ（`PENDING_APPROVAL` や `DRAFT` には使用しない）
...
3. **自分の現期目標サマリカード**（3目標のタイトルとステータス）
4. **前期最終評価**の表示
...
- ステータス履歴タイムライン
- **目標設定会議差し戻しボタン**（v1.3追加）: `APPROVED` 状態のときのみ MANAGER以上に表示。クリックで差し戻しコメント入力モーダルを表示し、送信すると `MEETING_REJECTED` に遷移
...
- タブ: 「承認待ち」「承認済み」「差し戻し済み」「会議差し戻し済み」「修正依頼済み」（v1.4追加）
- 申請種別: 目標設定承認 / 目標修正承認 / 修正依頼（v1.4追加） / 会議差し戻し（v1.3追加）
- 一覧に申請者・申請日・申請内容サマリを表示
- 承認・差し戻しボタン（差し戻し時はコメント入力必須）
- **会議差し戻し済み**タブ（v1.3追加）: 誰が・いつ・どのコメントで差し戻したかを一覧表示。社員が修正後に再申請すると「承認待ち」タブに移動する
...
POST   /api/goals/:goalSetId/meeting-reject     # 目標設定会議差し戻し（MANAGER以上・APPROVED時のみ）
...
- 期の途中での等級変更は原則として次期から適用するが、HR判断で当期適用も可能とする
