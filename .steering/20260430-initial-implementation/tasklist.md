...
- [x] `src/app/api/admin/audit-logs/route.ts` 作成（GET: HR/ADMIN のみ）
- [x] 承認・評価操作後の `audit_logs` 書き込み処理を各 Route Handler に組み込む

### P2機能

- [x] `src/app/(main)/goals/all/page.tsx` 作成（S-10 目標一覧・全社）
- [x] `src/app/(main)/employees/[employeeId]/page.tsx` 作成（S-11 社員プロフィール）
- [x] `src/app/(main)/reports/summary/page.tsx` 作成（S-16 評価サマリ）
- [x] `src/app/api/admin/degree360-scores/route.ts` 作成（POST）
- [x] `src/app/api/admin/degree360-scores/import/route.ts` 作成（CSV インポート）
- [x] `src/app/api/employees/[employeeId]/history/route.ts` 作成
- [x] `src/app/api/reports/summary/route.ts` 作成
